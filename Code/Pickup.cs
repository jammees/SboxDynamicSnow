...
public sealed class Pickup : Component
{
	...
	public PhysicsBody HeldObject
	{
		...
		set
		{
			if ( value is null )
			{
				bool hasRenderer = HeldObject.GameObject.Components.TryGet<ModelRenderer>( out var renderer );
				if ( !hasRenderer ) return;
				renderer.RenderType = ModelRenderer.ShadowRenderType.On;
			}
			else
			{
				bool hasRenderer = value.GameObject.Components.TryGet<ModelRenderer>( out var renderer );
				if ( !hasRenderer ) return;
